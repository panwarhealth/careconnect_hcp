...
- **Local WP-CLI OOMs at 128M since Rank Math (2026-07).** The wpcli container's PHP defaults to 128M and the plugin stack now exceeds it — plain `wp <cmd>` dies with a wp_die HTML page and exit 255. Always run `docker compose run --rm wpcli sh -c 'php -d memory_limit=512M "$(which wp)" <cmd>'` (same workaround as prod).
- **Local Apache lacks mod_headers by default.** The `X-Robots-Tag` PDF rules in `site/.htaccess` are wrapped in `<IfModule mod_headers.c>` and silently no-op until you run `docker compose exec wordpress a2enmod headers && docker compose exec wordpress apache2ctl graceful`. Re-run after recreating the wordpress container.
- **`site/robots.txt` is tracked in git since the SEO build (2026-07)** — it used to be gitignored as WP cruft. It and `site/.htaccess` remain deploy DENY-listed: prod copies are hand-edited over SSH with a `.bak`, never FTP'd.
